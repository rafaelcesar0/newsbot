...
async function performWebSearch(query: string): Promise<SearchResult[]> {
  ...
  if (lowerQuery.includes("brasil") || lowerQuery.includes("notícias") || lowerQuery.includes("política")) {
  ...
  if (lowerQuery.includes("inteligência artificial") || lowerQuery.includes("ia") || lowerQuery.includes("ai")) {
    ...
    const result = await streamText({
      ...
      tools: {
        searchWeb: tool({
          ...
          parameters: z.object({
            query: z.string().describe("A consulta de busca detalhada em português para encontrar informações na web"),
          }),
          execute: async ({ query }) => {
            console.log(`🔧 Ferramenta searchWeb executada com query: "${query}"`)
            try {
              const searchResults = await performWebSearch(query)
              const formattedResults = searchResults
                .map((r, index) => `${index + 1}. **[${r.title}](${r.link})**\n   ${r.snippet}`)
                .join("\n\n")
...
*Informações obtidas através de busca na web*`
              ...
              console.log("✅ Busca concluída, retornando resultados formatados")
              return result
            } catch (error) {
